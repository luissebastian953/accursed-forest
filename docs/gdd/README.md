...
This is where the design lives. The code cites it 378 times, from `GDD 2` to
`GDD 10.3`, so a reader who finds a number in `balance/` can get to the rule it
came from.

One file per top-level section, named for its number, so `GDD 3.6` is
